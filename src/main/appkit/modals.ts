import { localSearchQuerySchema, type LocalSearchResponse } from '../../shared/localSearch'
import type {
...
export class NativeModals {
  ...
  private searchContent(): NativeNode[] {
    ...
    const selected =
      response?.results.find((item) => item.id === this.selectedResult) ?? response?.results[0]
        ...
        ? [
            b.table(
              ...
              response.results.map((item) => ({
                id: item.id,
                title: item.title,
                subtitle: `${item.kind} · ${sourceDisplayName(item.source)} · ${item.createdAt.slice(0, 10)}`
              })),
              selected?.id ?? '',
              (id) => {
  ...
  private async search(): Promise<void> {
    ...
    try {
      ...
      if (!this.disposed && version === this.version) {
        ...
        this.selectedResult = response.results[0]?.id ?? ''
      }
    } catch (error) {
      if (!this.disposed && version === this.version) this.message = readableError(error)
    } finally {
