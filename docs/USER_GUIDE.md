...
- 搜索前后都可以直接在 Research question 输入框修改问题。编辑不会自动联网；按 Search 才执行。未提交的修改会显示 Draft not searched，当前结果继续保留。
- 点击结果或按 Return 直接阅读完整的已获取摘要，长内容自然滚动，不需要先点展开。完整来源字段及排序分值在 Source details 中保留；分值不是可信度或研究质量评分。
...
Command-F 打开 Find Local Research。输入至少两个字符后会自动筛选收藏、已保存的搜索会话和分析文本；按 Return 可以立即查找。它不调用模型或外部来源。

原生界面中点击结果或选中后按 Return 直接打开对应本地记录；历史分析按其固定 ID 打开。Back to search results 返回原查询及选择，并恢复原工作区中未提交的搜索草稿。Open original 是另一个动作，用于打开来源网站。记录失效时会说明原因，不会偷偷跳到一个不同的最新结果。

## 来源与统计

Sources 展示内置来源目录，并比较来源读取与 Discover 搜索，显示时间最新的有效记录。Latest search 使用已保存的搜索会话时间，Last source read 表示来源读取；这些是本地记录，不是实时可用性保证。No matches 表示该次查询没有匹配项，No items 表示来源窗口未返回条目。Failed or partial 可筛选失败或部分数据，详情保留原因；缓存内容单独展示。移动键盘焦点只预览来源；点击或按 Return 才打开内容。刷新失败会保留已有缓存，并更新失败状态。空筛选可以直接清除筛选。
