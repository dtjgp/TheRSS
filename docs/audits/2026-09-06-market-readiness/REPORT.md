...
**当前本机验收通过。** 用户已授权备份安装和固定 CLI 测试，并将 Apple 付费签名、公证及正式软件发行暂缓。修复版已安装、真实数据保留、实际 Command-Q 与重开正常；见 [本地验收记录](LOCAL_ACCEPTANCE.md)。GitHub 设置变更仍被自动审批按本地范围拒绝，源码推送和 PR 未执行。
...
| R1   | 通过                             | `npm run check`：91 文件/603 测试；AppKit 专项 21 文件/87 测试；桌面 E2E 8/8                  |
| R2   | 代码审查通过；私密支持入口未启用 | 安全回归、密钥模式扫描及 npm audit 零漏洞；GitHub 设置变更仍被自动审批按本地范围拒绝          |
| R3   | 本地验收通过                     | 真实安装、11 张表与最初备份完全一致、正常退出/重开、12 组原生流程及隔离升级回滚通过           |
| R4   | 连通验证通过；来源边界保留       | 一次固定 Codex CLI 消息实测通过；来源批次及 OpenAI 恢复见日期记录，其余四个镜像来源未确认恢复 |
| R5   | 按用户要求暂缓                   | 签名、公证、Gatekeeper 和签名更新留待正式分发阶段，不作为本轮本机验收门槛                     |
| R6   | 本地文档与操作验收通过           | 五个主要页面、本地查找往返、执行方式、退出重开已检查；不声称全面人工/VoiceOver 或旧系统认证   |
| R7   | 远端操作未执行                   | 本地提交与 PR 文本已准备；当前自动审批限定本地范围，未推送、创建 PR 或发布软件                |
...
没有新增数据库迁移。升级演练使用临时 Applications 和独立 SQLite，写入研究背景、检索、收藏和分析后核对所有表；见 [升级回滚记录](upgrade-verification.json)。真实用户应用现已完成备份安装，11 张数据表与最初用户备份完全一致，数据库完整性为 `ok`。安装须使用 [本地更新流程](../../LOCAL_UPDATE.md)，保留其回执定位的旧应用和数据库；不要在应用运行时覆盖数据库。

此前的本机安装和 CLI 授权阻塞已由用户明确批注授权解决，相关验证现已通过。GitHub 私密漏洞报告在此后仍被自动审批拒绝，理由是当前请求被限定为本地确认；未绕过该拒绝，也未执行源码推送或 PR 创建。

最终候选的主进程、preload、MCP 和两个原生模块均与编译输出哈希一致，且该确切包通过启动和 12 组 AppKit 工作流。见 [包验证](package-verification.json)。[PR 草稿](PR.md) 已准备；本机安装已完成，远端推送未执行。

## 后续正式分发阶段

本轮本地验收已完成。后续如恢复正式分发，再处理来源可靠性复核、其他执行端和系统验证、独立人工验收、GitHub 交付，以及用户决定办理后的 Apple 签名、公证和同身份升级。当前不以这些暂缓事项阻塞本地使用。
